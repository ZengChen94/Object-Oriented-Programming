...
public class MiniView<T> extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2604966376868305825L;
	...
	//		byte bytearray[] = null;
	//		try {
	//			FileInputStream fileinputstream = new FileInputStream(file);  //creat the incoming stream of data from the file
	//			int numberBytes = fileinputstream.available();  // find out how big the byte array needs to be.
	//			bytearray = new byte[numberBytes]; // allocate the byte array.
	...
	//		int returnVal = fileChooser.showSaveDialog(MiniView);
	...
	//					FileOutputStream fOutputStrm = new FileOutputStream(file);  // open the output stream.
	//					fOutputStrm.write(bytearray);  // write the byte array out to the disk.
